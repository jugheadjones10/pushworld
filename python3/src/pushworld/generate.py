...
def generate_puzzle(
    ...
    if num_goal_objects == 2:
        ...
        while goal_object_2_shape is None or goal_object_2_shape == goal_object_1_shape:
    ...
    place_object(puzzle, "A", shape=random.choice(possible_object_shapes))
...
def generate_braindead_puzzles(
    save_location_path: str,
    puzzle_size: int = 5,
):
    """Generates simple PushWorld puzzles with just an agent and a goal.

    This function systematically generates all possible permutations of agent and goal
    positions within a grid of the specified size, with no walls, obstacles, or
    movable objects other than the agent.

    Args:
        save_location_path: Directory to store generated puzzles. Must be empty or not exist.
        puzzle_size: The width and height of the puzzles to generate.
        object_shapes: Shape to use for both agent and goal. Defaults to 1x1.
    ...
    # Available positions are limited by the shape size
    available_positions = []
    ...
    # Generate all permutations of agent and goal positions
    for agent_pos in available_positions:
        for goal_pos in available_positions:
            # Skip cases where agent and goal would overlap
            ...
            puzzle[agent_y][agent_x] = "A"

            # Place goal
            goal_y, goal_x = goal_pos
            puzzle[goal_y][goal_x] = "G1"

            # Convert puzzle to string representation
            puzzle_string = "\n".join(["  ".join(l) for l in puzzle])

            # Save puzzle
            fname = f"{save_location_path}/braindead_puzzle_{puzzle_count}.pwp"
            with open(fname, "w+") as f:
                f.write(puzzle_string)

            puzzle_count += 1

    print(f"Generated {puzzle_count} puzzles at {save_location_path}")
    return puzzle_count
...
def generate_level0_puzzles(
    ...
    if min_num_walls < 0 or min_num_walls > max_num_walls:
        raise ValueError("min_num_walls must be >=0 and no bigger than max_num_walls")
...
def filter_puzzles_by_solvability(path: str, time_limit: int, num_puzzles: int):
    ...
    benchmark_rgd_planner(puzzles_path=path, time_limit=time_limit, results_path=path)
